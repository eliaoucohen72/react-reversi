...
function App() {
  ...
  const calculateScore = (board: Cell[], player: Player) =>
    board.filter((e) => e === player).length;
  const getScore = (player: Player) => useMemo(
    () => <div style={style.score}>{calculateScore(board, player)}</div>,
    [board, player] // Ajout du paramètre player comme dépendance
  );

  const renderedWhiteScore = getScore("white");
  const renderedBlackScore = getScore("black");
